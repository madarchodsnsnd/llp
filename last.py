...
async def set_vps_count_input(update: Update, context: CallbackContext):
    global ACTIVE_VPS_COUNT, USER_VPS_LIMITS
    
    user_id = update.effective_user.id
    try:
        count = int(update.message.text.strip())
        
        # Check if user is admin (unlimited VPS access)
        is_admin = user_id in ADMIN_IDS  # Assuming you have ADMIN_IDS defined
        
        # Get user's max allowed VPS count (default to 1 for normal users)
        max_allowed = USER_VPS_LIMITS.get(user_id, 1) if not is_admin else len(VPS_LIST)
        
        # Validate input
        if 1 <= count <= max_allowed:
            ACTIVE_VPS_COUNT = count
            
            # Store user's preference for future sessions
            USER_VPS_PREFERENCES[user_id] = count  # Add this global variable
            
            await update.message.reply_text(
                f"✅ *VPS Configuration Updated*\n"
                f"• Active VPS Count: *{count}*\n"
                f"• Your Max Allowed: *{max_allowed}*",
                parse_mode='Markdown'
            )
        ...
        logging.error(f"Error in set_vps_count_input: {str(e)}")
        await update.message.reply_text(
...
async def attack_input(update: Update, context: CallbackContext):
    ...
    # Check for available VPS (need exactly 4)
    ...
    if len(available_vps) < 4:
        current_display_name = get_display_name(update.effective_chat.id if update.effective_chat.type in ['group', 'supergroup'] else None)
        await update.message.reply_text(
            "❌ *All servers busy! Need exactly 4 free servers.*\n\n"
            f"👑 *Bot Owner:* {current_display_name}",
            parse_mode='Markdown'
        )
        return ConversationHandler.END
    ...
    # Set threads and validate duration
    ...
    def _run_ssh_attack(vps, threads_for_vps, attack_num, context):
        ...
        try:
            ...
            if not active_attacks:
                ...
                current_display_name = get_display_name(chat_id if update.effective_chat.type in ['group', 'supergroup'] else None)
